...
from gp import get_gradient, gradient_penalty
    ...
    for epoch in range(opt.max_epoch):
        ...
        for step, (img, _) in tqdm((enumerate(dataloader))):
            ...
            if step % opt.d_every == 0:
                ...
                # 将生成的图片交给判别模型进行判别
                crit_fake_pred = netd(fake_image)
                crit_real_pred = netd(real_img)

                epsilon = torch.rand(len(real_img), 1, 1, 1, device=device, requires_grad=True)
                gradient = get_gradient(netd, real_img, fake_image, epsilon)
